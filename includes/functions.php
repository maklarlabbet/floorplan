...

{
  ...
  "stairs": [ { "id": "s1", "x": 0, "y": 0, "width": 0, "height": 0, "orientation": "horizontal", "arrow_end": "start", "steps": 12 } ],
...
- "arrow_end" tells the renderer which corner of the bounding box the direction-of-travel arrow should point toward — "start" for the ("x","y") corner, or "end" for the opposite ("x"+"width","y"+"height") corner. Determine this by DIRECT OBSERVATION, not by translating the word "UP"/"DN" into a value — that kind of semantic translation is exactly the mistake that has caused wrong arrow directions before, so follow this concrete procedure instead:
  1. If the source has a drawn arrowhead/triangle on the staircase, just look at which corner of your bounding box it's closest to / pointing toward, and set "arrow_end" to that corner directly ("start" if closer to ("x","y"), "end" if closer to the opposite corner). That's it — no further reasoning needed.
  2. If the source only has text ("UP" or "DN"/"DOWN", no separate arrowhead), that text always sits at the entry point on THIS floor — the end where you'd start walking. The direction of travel is always AWAY from that text, toward the OTHER end. So: find which corner of your bounding box the text sits closest to, then set "arrow_end" to the OPPOSITE corner (if the text is near ("x","y"), set "arrow_end":"end"; if the text is near the opposite corner, set "arrow_end":"start"). This rule is the same regardless of whether the text says "UP" or "DN" — only the text's POSITION matters, never the word itself.
  3. If there is no arrowhead and no UP/DN text at all, make your best guess from context (e.g. which end connects to an open room vs. a wall) rather than omitting "arrow_end".
...
- Multi-flight staircases (turning 90°/180° at a landing, e.g. L-shaped or U-shaped): represent as MULTIPLE entries in "stairs" — one entry per straight flight, each with its own bounding box, "orientation", and "arrow_end" so each flight's arrow points the right way — plus one entry per landing (the flat turning pad between flights) with "steps": 0, which renders as a plain outline with no tread lines. A spiral/curved staircase should still be approximated this way (straight flights + landings); do not invent a curved shape.
- IMPORTANT ANTI-PATTERN: before writing a single "stairs" entry, check whether the run of steps (or its direction arrow) changes direction anywhere along its length — e.g. steps go one way, then turn and continue a different way, forming an L, U, or switchback shape. A single rectangular bounding box with one "orientation" can only ever represent a straight run; it CANNOT correctly represent a run that turns, no matter how you set "orientation"/"arrow_end". If the run turns even once, you MUST emit one "stairs" entry per straight flight plus one landing entry ("steps": 0) between each pair of flights, exactly as described above — never a single entry spanning the whole turning run. Treat collapsing a turning staircase into one entry as an error of the same severity as omitting the staircase entirely.
