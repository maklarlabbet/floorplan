...

{
  ...
  "stairs": [ { "id": "s1", "x": 0, "y": 0, "width": 0, "height": 0, "orientation": "horizontal", "arrow_point": {"x": 0, "y": 0}, "steps": 12 } ],
...
- "stairs": "x","y" is the top-left corner of the staircase's bounding box; "width"/"height" are its footprint; "orientation" is "horizontal" if the flight of steps runs left-right or "vertical" if it runs up-down the page; "steps" is the approximate number of treads.
...
- Winders (a compact way of turning a staircase using pie-shaped/tapered treads instead of a full rectangular landing) are drawn as a FAN of lines radiating out from a single point, usually attached to and continuing on from a straight run of parallel tread lines. This fan shape is easy to miss or mistake for decoration — it is still part of the SAME staircase, not a separate feature, and its full extent (including wherever the fan spreads to) belongs in the staircase's bounding box coverage. Do not stop the staircase's bounding box at the edge of the straight portion and ignore the fan section beyond it. Represent the winder itself as an additional "stairs" entry with "steps": 0 (rendered as a plain landing outline), positioned to cover the fan's footprint, following the same multi-flight rules above.
- IMPORTANT ANTI-PATTERN: before writing a single "stairs" entry, check whether the run of steps (or its direction arrow) changes direction anywhere along its length — e.g. steps go one way, then turn and continue a different way, forming an L, U, or switchback shape, OR a straight run of parallel treads transitions into a fan/winder pattern. A single rectangular bounding box with one "orientation" can only ever represent a straight run; it CANNOT correctly represent a run that turns, no matter how you set "orientation"/"arrow_point". If the run turns even once — including transitioning into a winder fan — you MUST emit one "stairs" entry per straight flight plus one landing entry ("steps": 0) between/after each flight, exactly as described above — never a single entry spanning the whole turning run or one that stops short of the winder. Treat collapsing a turning staircase into one entry, or omitting its winder section, as an error of the same severity as omitting the staircase entirely.
