...

{
  ...
  "stairs": [ { "id": "s1", "x": 0, "y": 0, "width": 0, "height": 0, "orientation": "horizontal", "arrow_point": {"x": 0, "y": 0}, "turn_at": null, "steps": 12 } ],
...
- "stairs": "x","y" is the top-left corner of the staircase's bounding box; "width"/"height" are its footprint — this box must cover the FULL visible extent of the staircase, including any winder/fan section (see below), not just the part with straight parallel treads. "orientation" is "horizontal" if the flight of steps runs left-right or "vertical" if it runs up-down the page; "steps" is the approximate number of treads in the straight portion only.
- "arrow_point" is a single {x,y} coordinate (same normalized units as everything else) marking a point at the END of the run that the stairs lead TOWARD — i.e. the destination end, not the end you'd start walking from. This is deliberately just a coordinate to read off the image, not a judgment call, so it should be easy to get right every time:
  1. If the source has a drawn arrowhead/triangle on the staircase, set "arrow_point" to that arrowhead's own approximate position (its tip). Nothing to figure out — just report where it is.
  2. If the source only has text ("UP" or "DN"/"DOWN", no separate arrowhead), that text sits at the entry point on THIS floor (where you'd start walking) — so set "arrow_point" to a coordinate near the OPPOSITE end of the run from the text (roughly the middle of the far end of the bounding box, away from the label). This is the one case requiring a small inference, but it's still just "put a point at the far end," not a corner name to look up.
  3. If there is no arrowhead and no UP/DN text at all, put "arrow_point" near whichever end connects to an open room rather than a wall, as a best guess.
  Do NOT try to convert this into words like "up"/"down"/"start"/"end" yourself — just give the coordinate and the renderer will work out the rest.
- "turn_at" (optional, {x,y} or null/omitted) marks where a straight run of parallel treads visibly changes into a winder fan (a cluster of lines radiating from a point — see below) partway along the SAME staircase. Put a coordinate roughly where that transition happens; leave it null/omitted if the whole run is uniform straight treads with no fan section. You do NOT need to figure out how to split this into separate JSON entries yourself, and you do NOT need to compute a second bounding box — just report the one overall box (already covering the full extent per the rule above) plus this single transition coordinate, and rendering will handle the rest. This is deliberately simple: one coordinate, not a structural decision.
- Staircases: look carefully for a staircase in the source — it's easy to mistake for plain hatching or a room divider. Stairs are drawn with several conventions, ALL of which count: (a) a series of closely spaced parallel lines ("treads") perpendicular to the direction of travel, (b) a diagonal crosshatch/herringbone texture filling the rectangle (common in Nordic/European plans — lines running corner-to-corner rather than straight across), or (c) in a system-made drawing, simply the label "Stairs"/"Staircase"/"Stairway"/"Trappa". Any of these, often paired with a straight or diagonal line down the middle (sometimes arrowed, sometimes labeled "UP"/"DN"). A small directional arrow (▲/▼ or similar) sitting on or beside a hatched/textured rectangular area is a strong staircase signal on its own, regardless of which hatch pattern is used — do not dismiss a hatched area as generic texture/fill just because the lines aren't axis-aligned. Whenever you find one — whether drawn as a symbol or only labeled with text — you MUST represent it as an entry in "stairs" with its bounding box; do NOT leave it out of the JSON, and do NOT represent it as a room name or a "notes" entry. IMPORTANT ANTI-PATTERN: if you catch yourself about to write a "notes" entry that mentions an arrow, an ascending/descending direction, or a staircase near some rectangular area — STOP. That is precisely the signal that a "stairs" entry belongs there instead; a prose description of the arrow's direction is never an acceptable substitute for an actual "stairs" entry with its bounding box, and doing so is treated as leaving the staircase out of the JSON. If you can identify the area but are unsure of the exact orientation/step count, make a reasonable best guess for those fields rather than omitting the entry — an approximate stairs entry is always better than none, and always better than describing it in "notes". Do not put the word "Stairs"/"Staircase" anywhere in a room "name" or in "notes".
- Winders: a compact way of turning a staircase using pie-shaped/tapered treads instead of a full rectangular landing, drawn as a FAN of lines radiating out from a single point, usually attached to and continuing on from a straight run of parallel tread lines. This fan is easy to miss or mistake for decoration — it is part of the SAME staircase. If you see one, use "turn_at" (above) to mark the transition point, and make sure the staircase's bounding box extends to cover the fan's full footprint too.
- Worked example — a staircase drawn as: a grid of parallel tread lines for its top half, then below that a fan of lines all radiating from one point near the middle, spreading down toward the bottom wall, with a small ▲ near the top of the grid pointing further up. The bounding box must cover BOTH the grid AND the fan (they are one staircase), and "turn_at" goes roughly where the grid ends and the fan begins:
  { "id": "s1", "x": 390, "y": 690, "width": 190, "height": 150, "orientation": "vertical", "steps": 7, "arrow_point": {"x": 480, "y": 695}, "turn_at": {"x": 480, "y": 750} }
  Notice "height" (150) extends all the way down to include the fan, not just the grid portion, and "turn_at" sits partway down the box, not at either edge — check your own bounding box the same way: if you can see a fan pattern anywhere within it (even at the very edge), "turn_at" must be set.
- Multi-flight staircases with a full rectangular landing (turning 90°/180°, e.g. L-shaped or U-shaped, where the turn is a flat rectangular pad rather than a winder fan): represent as MULTIPLE entries in "stairs" — one entry per straight flight, each with its own bounding box, "orientation", and "arrow_point" so each flight's arrow points the right way — plus one entry per landing (the flat turning pad between flights) with "steps": 0, which renders as a plain outline with no tread lines. A spiral/curved staircase should still be approximated this way (straight flights + landings); do not invent a curved shape.
- IMPORTANT ANTI-PATTERN: before writing a single "stairs" entry, check whether the run of steps (or its direction arrow) changes direction anywhere along its length — e.g. steps go one way, then turn and continue a different way, forming an L, U, or switchback shape, OR a straight run of parallel treads transitions into a fan/winder pattern (use "turn_at" for this specific case, as described above). A single rectangular bounding box with one "orientation" can only ever represent a straight run; it CANNOT correctly represent a run that turns, no matter how you set "orientation"/"arrow_point". If the turn is a rectangular landing, you MUST emit one "stairs" entry per straight flight plus one landing entry ("steps": 0) between/after each flight. If the turn is a winder fan, use "turn_at" on the single entry instead. Either way, never collapse a turning staircase into one entry that ignores the turn, and never let the bounding box stop short of covering the fan/landing area. Treat this as an error of the same severity as omitting the staircase entirely.
...
/**
 * Appends a cache-busting "?v=<mtime>" query string to a static asset path, based on the
 * file's last-modified time. Without this, browsers can keep serving a stale cached copy of
 * a JS/CSS file indefinitely after a deploy, with no visible sign anything is wrong — which
 * happened in practice (a deployed renderer update wasn't picked up until a manual hard
 * refresh). Use this instead of a bare path for any <script src> / <link href> in the app.
 */
function asset_url($rel_path) {
    $full_path = APP_ROOT . '/' . $rel_path;
    $version = is_file($full_path) ? filemtime($full_path) : time();
    return $rel_path . '?v=' . $version;
}
